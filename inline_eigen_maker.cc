...
AbsInlineMeasurement *createMeasurement2(XMLReader &xml_in, const std::string &path) 
{
...
bool registerAll(){
	...
	if(!registered){
		...
		success &= TheInlineMeasurementFactory::Instance().registerObject(name2, createMeasurement2);
		registered = true;
	}
	return success;
}
...
int check_eigen_name(std::string &name){
	...
	if(fd.good()) return 2;
	else if(fs.good()) return 1;
	else return 0;
}

//! Reader for parameters
void read(XMLReader &xml, const std::string &path, InlineEigenMakerParams::Param_t &param) 
{
...
void write(XMLWriter &xml, const std::string &path, const InlineEigenMakerParams::Param_t &param) 
{
	push(xml, path);
...
void read(XMLReader &xml, const std::string &path, InlineEigenMakerParams::NamedObject_t &input) 
{
...
void write(XMLWriter &xml, const std::string &path,	const InlineEigenMakerParams::NamedObject_t &input) 
{
	push(xml, path);
...
InlineEigenMakerParams::InlineEigenMakerParams(XMLReader &xml_in, const std::string &path) 
{
	try{
...
void InlineEigenMakerParams::write(XMLWriter &xml_out, const std::string &path){
	push(xml_out, path);
...
void InlineEigenMaker::func(unsigned long update_no, XMLWriter &xml_out){
	...
	if(TheNamedObjMap::Instance().check(params.named_obj.eigen_id)){
		...
		// If the object exists and is of the wrong type we will land here
			QDPIO::cerr << InlineEigenMakerEnv::name << ": caught dynamic cast error" << std::endl;
			QDP_abort(1);
		}
		catch (const std::string &e){
		// If the object is not in the map we will land here
			QDPIO::cerr << InlineEigenMakerEnv::name << ": map call failed: " << e << std::endl;
			QDP_abort(1);
		}
		// Bind gauge field reference to a local name
		const multi1d<LatticeColorMatrix> &u = TheNamedObjMap::Instance().getData<multi1d<LatticeColorMatrix> >(params.named_obj.gauge_id);
		...
		TheNamedObjMap::Instance().getData<EigenOperator<LatticeFermion> *>(params.named_obj.eigen_id) = eigen;
	...
	snoop.stop();
	QDPIO::cout << InlineEigenMakerEnv::name << ": total time = " << snoop.getTimeInSeconds() << " secs" << std::endl;
