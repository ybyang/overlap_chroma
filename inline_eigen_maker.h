...
template <typename T>
class EigenOperator: public UnprecLinearOperator<T, multi1d<LatticeColorMatrix>, multi1d<LatticeColorMatrix> >, public multi1d<EigenPair<T> > 
{
public:
	typedef multi1d<LatticeColorMatrix> P;
	typedef multi1d<LatticeColorMatrix> Q;
	// multi1d< EigenPair<LatticeFermion> >  eigenpair;
	virtual void general_dov(LatticeFermion& chi, const LatticeFermion& psi, double k0, double k1,double k2,double prec) const{};
	...
	int arnoldi_eigensystem(InlineEigenMakerParams &Params, Real kappa, int chiral=0){
		...
		shijian.stop();
		QDPIO::cout << "initial arnoldi factorization" << ": total time = " << shijian.getTimeInSeconds() << " secs" << std::endl;
		if(converge == dim){
			iteration = dim - nvec;
			for(;;){
				//main loop
				...
				shijian.stop();
				QDPIO::cout << "main loop step1" << ": total time = " << shijian.getTimeInSeconds() << " secs" << std::endl;
				//step2
				shijian.reset();
				shijian.start();
				...
				shijian.stop();
				QDPIO::cout << "main loop step2" << ": total time = " << shijian.getTimeInSeconds() << " secs" << std::endl;
				...
				shijian.stop();
				QDPIO::cout << "main loop step3" << ": total time = " << shijian.getTimeInSeconds() << " secs" << std::endl;
				...
				shijian.stop();
				QDPIO::cout << "main loop step4" << ": total time = " << shijian.getTimeInSeconds() << " secs" << std::endl;
                //main loop
			}
		}
		else yn = true;
		...
		shijian.stop();
		QDPIO::cout << "vector rotation" << ": total time = " << shijian.getTimeInSeconds() << " secs" << std::endl;
		for(int i=0;i<Noeigen;i++) es[i].vec=Qv[Index[i]];
	...
	void eigensystem(){
		...
		for(;iterations<Maxiteration;iterations++){
			...
			for(int iRow=0;iRow<idim;iRow++) Htemp(iRow,iRow) -= cEigen;
			QR(false,idim);
			for(int iRow=0;iRow<idim;iRow++) Htemp(iRow,iRow) += cEigen;
		}
	...
	void reconstruct(){
		...
		shijian.stop();
		QDPIO::cout << "reconstruct" << ": total time = " << shijian.getTimeInSeconds() << " secs" << std::endl;
	}
//eigen maker end
	...
	void save(const std::string filename, int io_num=1, bool to_single=false){
		...
		fileVec = NULL;
		shijian.stop();
		QDPIO::cout << "\nsave" << ": total time = " << shijian.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "write" << ": total time = " << VEC.para.readtime1.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "convert" << ": total time = " << VEC.para.readtime2.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "equal" << ": total time = " << VEC.para.readtime3.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "node" << ": total time = " << VEC.para.readtime4.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "route" << ": total time = " << VEC.para.readtime5.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "cpy" << ": total time = " << VEC.para.readtime6.getTimeInSeconds() << " secs" << std::endl;
	}
	...
	void load(std::string filename, int io_num=1, bool from_single=false){
		...
		fileVec = NULL;
		shijian.stop();
		QDPIO::cout << "\nload" << ": total time = " << shijian.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "read" << ": total time = " << VEC.para.readtime1.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "convert" << ": total time = " << VEC.para.readtime2.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "equal" << ": total time = " << VEC.para.readtime3.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "node" << ": total time = " << VEC.para.readtime4.getTimeInSeconds() << " secs" << std::endl;
		QDPIO::cout << "route" << ": total time = " << VEC.para.readtime5.getTimeInSeconds() << " secs" << std::endl;
...
public:
    ...
    // Erase the object
		QDPIO::cout << "Attempt to erase object name = " << params.named_obj.eigen_id << std::endl;
		write(xml_out, "object_id", params.named_obj.eigen_id);
		if(TheNamedObjMap::Instance().check(params.named_obj.eigen_id)){
			EigenOperator<LatticeFermion> *eigen = TheNamedObjMap::Instance().getData<EigenOperator<LatticeFermion> *>(params.named_obj.eigen_id);
			delete eigen;
			eigen = NULL;
			TheNamedObjMap::Instance().erase(params.named_obj.eigen_id);
		} 
		else
			QDPIO::cout << "Eigen system: " << params.named_obj.eigen_id << " is not in the map. Cannot delete" << std::endl;
