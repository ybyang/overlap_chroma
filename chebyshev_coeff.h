...
public:
  ...
  double zeta(double y) { return (2 * y - 1 - eps) / (1.0 - eps); }
  ...
  double root_residual(double y0, double y1, bool derivative = false) {
    ...
    if (fr * fl > 0)
      print0(
          "ERROR: find_zeros_error called with wrong ends: (%e %e)->(%e %e)\n",
          l, r, fr, fl);
  ...
  // Calculate the coefficients with given polynomial order n in the range
  // [lmin, lmax].
  bool compute_coef() {
    ...
    for (int iter = 0; iter < max_iter; iter++) {
      ...
      if (res <= prec) {
        //				print0("The struct cbxv_coef does
        //convergence
        // within %d iterations when N = %d, residual is
        //%.5e!\n",max_iter,N,res);
        return true;
      }
      if (iter == max_iter - 1) {
        //				print0("The struct cbxv_coef doesn't
        // convergence within %d iterations when N = %d, residual is
        //%.5e!\n",max_iter,N,res);
        return false;
      }
    }
...
public:
  ...
  void set_HwSize(int size, double _lmin = 1.0, double _prec = 1.0,
  ...
  chebyshev_coef(double _lmin, double _prec, bool print = false)
      : prec(_prec), eps(_lmin) {
    N = get_order();
    if (print == true)
      print0("N=%d\n", N);
    resize(N + 1);
